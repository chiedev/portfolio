...
function generateChord(request,instrument,element,chord_name){
	...
	**/
	...
	else if (request == 'chord' || request == 'chord'){
	...
	if (type == 'tooltip'){
		...
		.success(function( result ) {
			...
			for (var i = 0; i <= obj.length - 1; i++) {
				if ( obj[i].chord_name === chord_name && (instrument == 'guitar' && obj[i].chord_code != '') ||
				     obj[i].chord_name === chord_name && (instrument == 'ukulele' && obj[i].chord_code_ukulele != '') ){
					code = (instrument == 'guitar')?obj[i].chord_code:obj[i].chord_code_ukulele
			...
			if (counter == 0){
				element.append('<container hidden class="chord-tooltip">No Available Chord</container>')
			}
			counter = 0
			...
			element.on('mouseleave',function(){
				$('container',this).delay(200).fadeOut(0)
			});
		})
	}
	else if (type == 'chord'){
	    ...
	    .done(function( result ) {
			...
			for (var k = 0; k <= chord_name.length - 1; k++) {
				for (var i = 0; i <= obj.length - 1; i++) {
					if (chord_name.length == 1) {
						...
						if (((obj[i].chord_name.match(chord_name[k]) != null) && (instrument == 'guitar' && obj[i].chord_code != '')) ||
						 ((obj[i].chord_name.match(chord_name[k]) != null) && (instrument == 'ukulele' && obj[i].chord_code_ukulele != '')) ){
							var code = (instrument == 'guitar')?obj[i].chord_code:obj[i].chord_code_ukulele
						...
						if (((obj[i].chord_name == chord_name[k]) && (instrument == 'guitar' && obj[i].chord_code != '')) ||
						 ((obj[i].chord_name == chord_name[k]) && (instrument == 'ukulele' && obj[i].chord_code_ukulele != '')) ){
							var code = (instrument == 'guitar')?obj[i].chord_code:obj[i].chord_code_ukulele
							var name = obj[i].chord_name
							chords.push({"code":code,"name":name})
						}	
			...
			if (chords.length > 0){
				...
				element.append('<div id="chord-error">'
							 ...
							 +'<p>No available chord for now.</p>'
